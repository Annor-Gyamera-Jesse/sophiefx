...
async function attributeAffiliate(bookingRef, amountPesewas, tier, metadata) {
  const affiliateCode = metadata?.affiliate_ref;
  if (!affiliateCode) {
    console.log('[verify] no affiliate_ref in metadata — skipping attribution');
    return;
  }
  ...
  try {
    // Look up approved affiliate by code
    ...
    if (!getRes.ok) { console.warn('[verify] affiliate lookup failed:', getRes.status); return; }

    const rows = await getRes.json();
    if (!rows || !rows.length) {
      console.warn('[verify] affiliate not found or not approved:', affiliateCode);
      return;
    }
    ...
    // Update affiliate earnings + booking count
    ...
    // Tag the booking row with the affiliate code
    if (bookingRef) {
    ...
    console.log(`[verify] ✓ affiliate ${affiliateCode} earns GH₵${commission} for booking ${bookingRef}`);

  } catch (err) {
    console.error('[verify] affiliate attribution error (non-fatal):', err.message);
  }
}